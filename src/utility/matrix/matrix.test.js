...
it('accepts a 2D array as an initial value', () => {
  ...
  m.forEach((cell, [rowIndex, colIndex]) => {
    expect(twoD[rowIndex][colIndex]).toEqual(cell);
  });
});

it('accepts height, width, and an initial value to create', () => {
  expect(new Matrix(2, 2, 'hi')).toEqual(new Matrix([['hi', 'hi'], ['hi', 'hi']]));
});
...
it('allows you to map over each value', () => {
  const m = new Matrix([[1, 2, 3], [4, 5, 6], [7, 8, 9]]);
  const expected = new Matrix([[2, 4, 6], [8, 10, 12], [14, 16, 18]]);
...
it('slices a window of the matrix', () => {
  const m = new Matrix([[1, 2, 3], [4, 5, 6], [7, 8, 9]]);
  const expected = new Matrix([[2, 3], [5, 6]]);
